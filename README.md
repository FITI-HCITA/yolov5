...
- Please check your local model path **-w "pretrained pytorch model path"**
    ...
    path: ``VA8801_Model_Zoo/ClassicalModels/resnet/ObjectDetection``

```bash
python tflite_runtime.py -s data/dataset/test/esl_001.jpg -w path/ResNet_2.001.001.tflite --img_ch 3
```
3.  Train model: Transfer learning with a PyTorch pretrained model, which can be downloaded from the model zoo for the [ResNet model](https://github.com/FITI-HCITA/VA8801_Model_Zoo/blob/main/ClassicalModels/resnet/ObjectDetection/ResNet_2.001.001.pt)
...
```bash
python train.py --device 0 --data data/training_cfg/data_config.yaml --weights path/ResNet_2.001.001.pt --imgsz 320 --imgch 3 --cfg models/resnet50.yaml
```
...
```bash
python ai_pipeline.py --data data/training_cfg/data_config.yaml --weights path/ResNet_2.001.001.pt --batch-size 1 --imgch 3 --imgsz 320 --imgsz_tflite 320 --device 0 --include tflite --int8 --run export
