...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            class_conf = torch.cat((conf, x[:, 5:]), 1)[
...
# copy from yolov5.utils.general
def non_max_suppression_objcls(
    prediction,
    class_idx_gt=0,
    conf_thres=0.25,
    iou_thres=0.45,
    ...
    xc = (
        torch.max(prediction[..., 4:5] * prediction[..., 5:], dim=2).values
...
def get_video_path_dir(dir_path: str):
    ...
    return [
        ...
        if os.path.isfile(os.path.join(dir_path, f)) and f.endswith(extensions)
    ]


def infer_video(
    model: TFLiteModel, media_source: str, media_type, confThr: float = 0.4
):
        ...
        while True:
            ...
            if ret:
                ...
                for i in range(bndboxes.shape[0]):
                    ...
                    cv2.rectangle(
                        frame, (x0, y0), (x0 + 10, y0 + 10), (0, 0, 0), -1, cv2.LINE_AA
                    )  # filled
                    cv2.putText(
                        frame,
                        str(cls_idx),
...
def infer_images(
    ...
    **kwargs,
    ...
    assert os.path.isfile(
        tflite_model_path
    ), f"tflite model file: {tflite_model_path} is not exist."
    ...
    for cur_idx in range(num_images):
        ...
        bndboxes, class_confs, upd_tabel = non_max_suppression_objcls(
            y, class_idx_gt, conf_thres=conf_thres, iou_thres=iou_thres
        )
        # print("bndboxes:", bndboxes)
        ...
        for i in range(bndboxes.shape[0]):
            ...
            cv2.rectangle(
                image, (x0, y0), (x0 + 10, y0 + 10), (0, 0, 0), -1, cv2.LINE_AA
            )  # filled
            cv2.putText(
                image,
                str(cls_idx),
            ...
            cv2.rectangle(
                enlarge_image, (x0, y0), (x0 + 10, y0 + 10), (0, 0, 0), -1, cv2.LINE_AA
            )  # filled
            cv2.putText(
                enlarge_image,
                str(cls_idx),
    ...
    if generate_conf_excel:
        excel_path = os.path.join(
            save_dir,
            f"robot_Photo_inference_obj_conf{conf_thres}_{str(Path(tflite_model_path).stem)}.xlsx",
        )
        ...
        for conf_idx in range(10, -1, -1):
            for class_idx in range(num_classes):
                ...
                wrong_cnt[class_idx] += (
                    np.sum(table[:, conf_idx, class_idx])
                    - table[class_idx, conf_idx, class_idx]
                )

                if num_data_per_class[class_idx]:
                    recall = correct_cnt[class_idx] / num_data_per_class[class_idx]
                    object_view = (
                        correct_cnt[class_idx] + wrong_cnt[class_idx]
                    ) / num_data_per_class[class_idx]
                else:
                    recall = 0
                    object_view = 0
            ...
            summary_table[-1, conf_idx] = np.sum(correct_cnt) / np.sum(
                num_data_per_class
            )
            ...
            multi_index = pd.MultiIndex.from_product(
                multi_index, names=["Confidence", "Acutal"]
            )
        ...
        summary_sheet.columns = pd.MultiIndex.from_product(
            multi_index_columns, names=["Confidence"]
        )
        multi_index = [[], []]
        for class_idx in range(num_classes):
        ...
        multi_index = pd.MultiIndex.from_product(
            multi_index, names=["Class Name", "Measurement"]
        )
        multi_index = multi_index.append(
            pd.MultiIndex.from_arrays([["All"], ["Accuracy"]])
        )
        ...
        for column in summary_sheet:
            column_length = max(
                summary_sheet[column].astype(str).map(len).max(), len(column)
            )
            col_idx = summary_sheet.columns.get_loc(column)
            f_excel.sheets["Summary"].set_column(col_idx, col_idx, column_length + 10)

        confusion_matrix_sheets = pd.concat(
            [
                confusion_matrix_sheet
                for confusion_matrix_sheet in confusion_matrix_sheets
            ],
            axis=0,
        )
        ...
        for column in confusion_matrix_sheets:
            column_length = max(
                confusion_matrix_sheets[column].astype(str).map(len).max(), len(column)
            )
            col_idx = confusion_matrix_sheets.columns.get_loc(column)
            f_excel.sheets["Confusion Matrix"].set_column(
                col_idx, col_idx, column_length + 10
            )
        ...
        for class_idx in range(num_classes):
            ...
            table_pd.columns = pd.MultiIndex.from_product(
                multi_index_columns, names=["Confidence"]
            )
            table_pd.to_excel(
                f_excel, sheet_name=classIndexName[class_idx], na_rep="NaN"
            )

            # auto-adjust column width
            for column in table_pd:
                column_length = max(
                    table_pd[column].astype(str).map(len).max(), len(column)
                )
                col_idx = table_pd.columns.get_loc(column)
                f_excel.sheets[classIndexName[class_idx]].set_column(
                    col_idx, col_idx, column_length + 10
                )
    ...
    if generate_sa_txt:
        sa_txt_save_name = os.path.join(
            save_dir, f"ROT_infer_{str(Path(tflite_model_path).stem)}.txt"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--type", type=str, default="image", help="image or video"
    )
    parser.add_argument(
        "-s",
        "--source",
        type=str,
        help="source for image file or folder path of image or video file path or video device id",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable Store image inference result",
    )
    parser.add_argument(
        "--store_video_result",
        action="store_true",
        help="Enable Store video inference result",
    )
    parser.add_argument(
        "--show_cls_conf", action="store_true", help="Enalbe Show all class confidence"
    )
    ...
    parser.add_argument(
        "--get_mismatch",
        action="store_true",
        help="get prediction and ground truth mismatch, only for image",
    )
    parser.add_argument(
        "--append_mismatch_str",
        action="store_true",
        help="append mismatch string when get mismatch image",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="generate object confidence statistics excel",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="generate SA images inference imformation txt",
    )
    ...
    if args.type == "video":
        ...
        if os.path.isdir(MEDIA_SOURCE):
            ...
            if (
                MEDIA_SOURCE.endswith("jpeg")
                or MEDIA_SOURCE.endswith("jpg")
                or MEDIA_SOURCE.endswith("png")
                or MEDIA_SOURCE.endswith("bmp")
            ):
    ...
    if (
        ...
        print(
            "VIDEO INFER STORE: {}".format("ON" if ENABLE_STORE_VIDEO_RESULT else "OFF")
        )
        print(
            "GENERATE CONFIDENCE EXCEL: {}".format(
                "ON" if ENABLE_GENERATE_CONF_EXCEL else "OFF"
            )
        )
        print("GENERATE SA TXT: {}".format("ON" if ENABLE_GENERATE_SA_TXT else "OFF"))
        print("==================================")
        print(
            "GET MISMATCH              : {}".format("ON" if IS_GET_MISMATCH else "OFF")
        )
        print(
            "APPEND MISMATCH STRING    : {}".format(
                "ON" if APPEND_MISMATCH_STRING else "OFF"
            )
        )
        print(
            "GET YOLO LABEL            : {}".format(
                "ON" if IS_GET_YOLO_LABEL else "OFF"
            )
        )
